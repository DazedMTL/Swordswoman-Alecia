...
(() => {
  ...
  const PageNextSymbol = NuunManager.getStringCode(
    parameters["PageNextSymbol"]
  );
  const PagePreviousSymbol = NuunManager.getStringCode(
    parameters["PagePreviousSymbol"]
  );
  const PageList =
    (NUUN_Base_Ver >= 113
  ...
  const EXPGaugeColor1 =
    DataManager.nuun_structureData(parameters["EXPGaugeColor1"]) || 0;
  const EXPGaugeColor2 =
    DataManager.nuun_structureData(parameters["EXPGaugeColor2"]) || 0;
  ...
  const ElementRadarChartFramecolor =
    DataManager.nuun_structureData(parameters["ElementRadarChartFramecolor"]) ||
    0;
  const ElementRadarChartLineColor =
    DataManager.nuun_structureData(parameters["ElementRadarChartLineColor"]) ||
    0;
  const ElementRadarChartMainColor1 =
    DataManager.nuun_structureData(parameters["ElementRadarChartMainColor1"]) ||
    0;
  const ElementRadarChartMainColor2 =
    DataManager.nuun_structureData(parameters["ElementRadarChartMainColor2"]) ||
    0;
  ...
  const StateRadarChartFramecolor =
    DataManager.nuun_structureData(parameters["StateRadarChartFramecolor"]) ||
    0;
  const StateRadarChartLineColor =
    DataManager.nuun_structureData(parameters["StateRadarChartFramecolor"]) ||
    0;
  const StateRadarChartMainColor1 =
    DataManager.nuun_structureData(parameters["StateRadarChartMainColor1"]) ||
    0;
  const StateRadarChartMainColor2 =
    DataManager.nuun_structureData(parameters["StateRadarChartMainColor2"]) ||
    0;
  ...
  Scene_Status.prototype.createStatusWindow = function () {
    ...
    if (this.maxPage() > 1 && !!PageNextSymbol && !!PagePreviousSymbol) {
      this._statusWindow.setHandler(
        PageNextSymbol,
        this.statusPageup.bind(this)
      );
      this._statusWindow.setHandler(
        PagePreviousSymbol,
        this.statusPagedown.bind(this)
      );
    }
    if (BackGroundImg) {
  ...
  Scene_Status.prototype.createStatusButton = function () {
    if (this.maxPage() > 1 && ConfigManager.touchUI) {
      ...
      if (
        this.arePageButtonsEnabled() &&
        this._pageupButton.x > Graphics.width / 2
      ) {
        this._pageupButton.x -=
          40 + this._pageupButton.width + this._pagedownButton.width;
        this._pagedownButton.x -=
          40 + this._pageupButton.width + this._pagedownButton.width;
      }
      const y = this.buttonY();
      this._statusupButton.x =
        this._pageupButton.x +
        (this.arePageButtonsEnabled()
      ...
      switch (list.DateSelect) {
        ...
        case 301:
          ...
          if (meta) {
            ...
            if (dataImg) {
              loadBitmap = ImageManager.loadBitmap(
                "img/" + ImgFolder + "/",
                dataImg[0]
              );
            }
    ...
    if (this._actorBitmap) {
      const name =
        Imported.NUUN_ActorPicture && ActorPictureEXApp
          ...
          : data.ActorImg[actor.statusImgId];
      this._actorBitmap.setup(actor, data, name);
      this._actorBitmap.move(
        rect.x + data.Actor_X,
        rect.y + data.Actor_Y,
        rect.width,
        rect.height
  ...
  Window_Status.prototype.drawMastery = function (list, actor, x, y, width) {
    try {
      ...
      const log = $gameSystem.isJapanese()
        ? "AvPort_dsWeaponMastery.jsが見つかりません。"
        : "AvPort_dsWeaponMastery.js not found.";
      throw ["LoadError", log];
    }
  };
  ...
  Window_Status.prototype.drawActorLevel = function (actor, x, y, width, list) {
    ...
    this.drawText(
      text,
      x + systemWidth + this.itemPadding(),
      y,
      width - (systemWidth + this.itemPadding()),
      list.Align || "right"
    );
  };
  ...
  Window_Status.prototype.drawEquip = function (list, actor, x, y, width) {
    ...
    for (let i = 0; i < e1uipsLength; i++) {
      ...
      if (
        slotName &&
        (!showEquips ||
          (showEquips && showEquips.some((data) => data === slotName)))
      ) {
        let sw = 0;
        let iconWidth = 0;
        if (list.Back) {
        ...
        if (EquipNameVisible > 1) {
          ...
          iconWidth =
            ImageManager.iconWidth + (EquipNameVisible === 2 ? 24 : 4);
        }
  ...
  Window_Status.prototype.drawDesc = function (list, actor, x, y, width) {
    ...
    const text =
      list.DetaEval === "actorclass"
        ? actor.currentClass().meta[method]
        : actor.actor().meta[method];
    if (!!text) {
    ...
    if (ElementResist) {
      ...
      for (let i = 0; i < elementResistlength; i++) {
        ...
        if (elementId > 0) {
          ...
          this.drawText(
            rateText,
            x2 + systemWidth + this.itemPadding(),
            y2,
            width2 - (systemWidth + this.itemPadding()),
            list.Align || "right"
          );
        }
      }
  ...
  Window_Status.prototype.drawExpInfo = function (list, actor, x, y, width) {
    ...
    const expTotal = !list.ParamName
      ? TextManager.expTotal.format(TextManager.exp)
      : list.ParamName;
    const lineHeight = this.lineHeight();
    this.changeTextColor(NuunManager.getColorCode(list.NameColor));
  ...
  Window_Status.prototype.drawExpGaugeInfo = function (
    ...
    const expNext = !list.ParamName
      ? TextManager.expNext.format(TextManager.level)
      : list.ParamName;
    this.changeTextColor(NuunManager.getColorCode(list.NameColor));
  ...
  Window_Status.prototype.drawStatusCommonImg = function (
    list,
    actor,
    x,
    y,
    ...
    if (bitmap && !bitmap.isReady()) {
      bitmap.addLoadListener(
        this.drawImg.bind(this, bitmap, list, x, y, width)
      );
    } else if (bitmap) {
  ...
  Window_Status.prototype.nuun_setContentsFontFace = function (list) {
    this.contents.fontFace = list.FontFace
      ? list.FontFace
      : list.ValueFontFace
  ...
  if (Imported.dsWeaponMastery) {
    const _Scene_Status_createStatusMasteryWindow =
