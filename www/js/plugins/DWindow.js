...
(function () {
  ...
  // パラメータの取得と整形
  //=============================================================================
  var paramGameVariablesXPos = getParamNumber(
    ["GameVariablesXPos", "X座標の変数番号"],
    0
  );
  var paramGameVariablesYPos = getParamNumber(
    ["GameVariablesYPos", "Y座標の変数番号"],
    0
  );
  var paramGameVariablesWidth = getParamNumber(
    ["GameVariablesWidth", "横幅の変数番号"],
    0
  );
  var paramGameVariablesHeight = getParamNumber(
    ["GameVariablesHeight", "高さの変数番号"],
    0
  );
  var paramAlwaysOnTop = getParamBoolean(["AlwaysOnTop", "最前面に表示"]);
  var paramIncludePicture = getParamNumber(
    ["IncludePicture", "ピクチャに含める"],
    0
  );
      ...
      case "D_WINDOW_DRAW":
        switch (args.length) {
          ...
          case 2:
            ...
            windowInfo.width =
              $gameVariables.value(paramGameVariablesWidth) || 0;
            windowInfo.height =
              $gameVariables.value(paramGameVariablesHeight) || 0;
            windowInfo.opacity =
              args.length > 1 ? getArgNumber(args[1], 0, 255) : 255;
            break;
          case 5:
          case 6:
            number = getArgNumber(args[0], 1, 10);
            ...
            windowInfo.opacity =
              args.length > 5 ? getArgNumber(args[5], 0, 255) : 255;
            break;
          default:
            throw new Error(
              command + "に指定した引数[" + args + "が不正です。"
            );
        }
    ...
    for (var i = 0; i < 10; i++) {
      ...
      if (paramIncludePicture > 0) {
        ...
        if (this._pictureContainerLower) {
          ...
          this.addDynamicWindowForPicturePriority(
            this._pictureContainerLower,
            dWindow
          );
          this.addDynamicWindowForPicturePriority(
            this._pictureContainerMiddle,
            dWindow
          );
          this.addDynamicWindowForPicturePriority(
            this._pictureContainerUpper,
            dWindow
          );
        } else {
          this._pictureContainer.addChildAt(
            this._DynamicWindows[i],
            paramIncludePicture + i
          );
        }
      } else {
  ...
  Spriteset_Base.prototype.addDynamicWindowForPicturePriority = function (
    pictureLayer,
    dWindow
  ) {
