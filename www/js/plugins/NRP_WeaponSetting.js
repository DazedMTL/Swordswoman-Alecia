...
(function () {
  ...
  if (pSupportOver30Image) {
     ...
     * ●武器アニメーションの開始
     */
    const _Game_Battler_startWeaponAnimation =
  ...
  function changeAnimationColor(sprite, cell, spriteAnimation) {
    ...
    if (pattern >= 0 && subject) {
      ...
      const animation1 = subject.attackAnimationId1
        ? subject.attackAnimationId1()
        : 0;
      const animation2 = subject.attackAnimationId2
        ? subject.attackAnimationId2()
        : 0;

      // 武器のアニメーションではない場合
      if (
        spriteAnimation._animation.id != animation1 &&
