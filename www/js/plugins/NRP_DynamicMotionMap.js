...
(function () {
  ...
  Sprite_Character.prototype.updateDynamicMotion = function () {
    ...
    while (this.isDynamicMotionRequested()) {
      ...
      if (
        !this._dynamicMotionDuration ||
        dynamicMotion.maxDuration > this._dynamicMotionDuration
      ) {
   ...
   */
  const _Sprite_Character_startDynamicMotion =
    Sprite_Character.prototype.startDynamicMotion;
   ...
   */
  const _Game_Interpreter_isDynamicAnimationPlaying =
    Game_Interpreter.prototype.isDynamicAnimationPlaying;
  ...
  Sprite_Character.prototype.startMoveDynamic = function (x, y, duration) {
    ...
    if (character.hasWalkAnime() && !motion.evalPattern) {
      ...
      // 座標移動がある場合のみ対象
      if (
        character._x != character._realX ||
        character._y != character._realY
      ) {
        // ジャンプ以外
   ...
   */
  const _Sprite_Character_updateDynamicMove =
    Sprite_Character.prototype.updateDynamicMove;
  ...
  Sprite_Character.prototype.decimalMapY = function () {
    ...
    if (Utils.RPGMAKER_NAME == "MV") {
      return $gameMap.canvasToDecimalMapY(
        this.y -
          $gameMap.tileHeight() +
    ...
    // ＭＺの場合
    return $gameMap.canvasToDecimalMapY(
      this.y - $gameMap.tileHeight() + this._character.jumpHeight()
    );
  };
   ...
   * ●ジャンプの高さ
   */
  const _Game_CharacterBase_jumpHeight =
   ...
   */
  const _Sprite_Character_updateDynamicPosition =
    Sprite_Character.prototype.updateDynamicPosition;
  ...
  Game_CharacterBase.prototype.targetScreenY = function () {
    ...
    return Math.floor($gameMap.adjustY(this._y) * th + th - this.shiftY());
  };
  ...
  Sprite_Character.prototype.getNearCoordinate = function (b) {
    ...
    // ※縦側は足元が基準点なので調整
    if (
      Math.abs(a.x - b.x) < (widthA + widthB) / 2 &&
      Math.abs(a.y - heightA / 2 - (b.y - heightB / 2)) <
        (heightA + heightB) / 2
    ) {
      const now = [];
      now.x = a.x;
  ...
  Spriteset_Map.prototype.saveMotionTempData = function () {
    ...
    if (Nrp.pKeepAnimation) {
      ...
      $gameTemp.motionSprites = this._characterSprites.filter(function (
        sprite
      ) {
  ...
  function getSprite(target) {
    ...
    if (!$gameParty.inBattle()) {
      return spriteset._characterSprites.find(
        (sprite) => sprite._character == target
      );
      // 戦闘中はバトラースプライトを返す。
    } else {
      return spriteset
        .battlerSprites()
        .find((sprite) => sprite._battler == target);
    }
  }
  ...
  if (Game_CharacterBase.prototype.initEffects) {
    const _Game_CharacterBase_initEffects =
      Game_CharacterBase.prototype.initEffects;
    ...
    const _Sprite_Character_updateAngle =
  ...
  if (Sprite_Character.prototype.updateExtend) {
    const _Game_CharacterBase_clearCgInfo =
      Game_CharacterBase.prototype.clearCgInfo;
    ...
    const _Sprite_Character_updateExtend =
      Sprite_Character.prototype.updateExtend;
